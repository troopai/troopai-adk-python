...
description: >-
  ...
  one scope per call.
whenToUse: Completing or standardizing docstrings in one file or directory.
tools:
  - Read
  - Edit
  - Write
...
2. **No memory-layer leakage.** Never write `.claude/`, `.kimi-code/`,
   `CLAUDE.md`, `AGENTS.md`, a rule basename, or a governance pointer
   ("per the X rule", "see X") into a docstring. If you SEE existing
   leakage, report it under `leakage_spotted` — do not fix it (another
   workstream owns that).
