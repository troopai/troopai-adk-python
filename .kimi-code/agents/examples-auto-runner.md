...
whenToUse: Running the examples suite and fixing failing examples end-to-end.
tools:
  - Read
  - Edit
  - Write
...
Related skills you should invoke when their trigger arrives: **run-examples**
(suite operation) and **ruff-format-code** (formatting changed files).
...
5. **No version language, no memory-layer leakage** in example edits: no
   `v1`/`v2`/`Phase N`/`legacy`; never write a `.claude/` path, a
   `.kimi-code/` path, a `CLAUDE.md` / `AGENTS.md` reference, or a rule
   title into example code or prose.
